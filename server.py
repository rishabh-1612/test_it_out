...
from utils.file_retrieval_utils import InputPayload, check_old_file_data, OrderedOutputBasic, OrderedOutputTechnical, convert_old_file_data_to_new
from flask import Flask, Response, request, jsonify, make_response, Blueprint, send_file, stream_with_context, url_for, redirect
...
from helper_func import (
    ...
    save_architecture_and_services, save_architecture_and_services_v2, get_architecture_fetch_flow,
...
setting_config_collection = db["setting_config"]

redis_pool = redis.ConnectionPool(
        host=config.REDIS_HOST,
        port=config.REDIS_PORT,
        password=config.REDIS_PASSWORD,
        socket_connect_timeout=1,  # 1 second timeout to connect
...
REDIS_PREFIX = "setting_config:"
mongo_reddis_config = {"setting_config_collection":setting_config_collection, "REDIS_PREFIX":REDIS_PREFIX, "r":r}
...
def get_assistant_id_v2():
    try:
        ...
        docs = project_summery_db.projects.find({"repo_urls": {"$in": normalized_urls}})
        docs = list(docs)
        if not docs:
            # Check with repo name if not found with git_url
            if git_url.endswith('.git'):
                ...
                repo_name = git_url.split('/')[-1]
            docs = project_summery_db.projects.find({"repo_urls": {"$regex": repo_name, "$options": "i"}})
            if not docs:
                return jsonify({"error": "No matching git_url found"}), 404
        owner_docs = []
        owner_docs = [doc for doc in docs if doc.get('user_id') == user_id]

        today_date = str(datetime.now(timezone.utc).date())
         # Step 1: Fetch User Repos with Edit Access
        user_repos = project_summery_db.user_repo_access.aggregate([
            {"$match": {"user_id": user_id}},
            {"$unwind": "$projects"},
            {"$match": {
                "projects.access_type": "edit",
                "projects.validity": {"$gte": today_date}
            }},
            {"$project": {"project_id": "$projects.project_id"}}
        ])

        project_ids = [repo["project_id"] for repo in user_repos]   
        # Step 2: Filter the documents based on the project IDs
        edit_docs = []
        edit_docs = [doc for doc in docs if doc.get('project_id') in project_ids]   


        # Step 3: Combine the results and remove duplicates based on 'task_id'
        combined_docs = owner_docs + edit_docs
        seen_task_ids = set()
        final_docs = []
        ...
        for document in final_docs:
            ...
            assistant_id = document.get('assistant_id')
            task_id = document.get('task_id')
            branch_name = document.get('branch_names')
            project_name = document.get('project_name')
            project_id = document.get('project_id', "")
            git_info = document.get("git_info", [])
            commit_hash = ""
            if len(git_info) > 0:
                git_info = git_info[0]
            ...
            repo_names = document.get('repo_urls', [])
            match = re.search(r'github\.com[:/][^/]+/([^/]+?)(?:\.git)?(?:[/?#]|$)', repo_names[0])
...
def project_summary_sync_request():
    ...
    try:
        ...
        request_data['analysis_type'] = "sync"  #or new analysis or re-analysis
...
def local_sync_ecg():
    ...
    try:
        ...
        dependency_thread = threading.Thread(
            target=lambda: dependency_result.append(local_sync_dependency_graph(user_id, task_id, repo_url, base_branch_name, operations))
        )

        feature_thread = threading.Thread(
            target=lambda: feature_result.append(local_sync_feature_hierarchy(task_id, operations))
        )
        ...
        updated_folder_structure, _ = local_sync_update_folder_paths(repo_url, operations)
...
def share_default_project():
    ...
    projects = new_user_default_data
